...
return (
    <main className="py-4  ps-6 md:px-10 lg:px-20 flex flex-col gap-10">
        <div className="text-center">
            ...
            <h1 className="md:text-[40px] text-[30px] font-bold">Our Resources</h1>
        </div>
        <div className="flex gap-5 rounded-lg overflow-auto">
            {ResourcesDetails?.map((item, index) => {
                return (
                    <div key={index} className="rounded-lg border-1 border-gray-200 w-[100%]">
                        <div className="w-80">
                            <img src={item.img} alt="" className="rounded-tr-lg rounded-tl-lg mb-4" />
                            <div className="px-2 pb-4">
                                <h1 className="text-left text-[16px] font-bold mb-2">{item.title}</h1>
                                <p className="text-left mb-4 text-[16px] text-gray">{item.content}</p>
